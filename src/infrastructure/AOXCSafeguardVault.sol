...
contract AOXCSafeguardVault is ReentrancyGuard {
    ...
    function releaseCompensation(address _victim, uint256 _amount) external nonReentrant {
        if (msg.sender != commander && msg.sender != SOVEREIGN) revert AOXCErrors.Unauthorized(msg.sender);
        if (_victim == address(0)) revert AOXCErrors.InvalidRecipient();
        if (address(this).balance < _amount) revert AOXCErrors.InsufficientReserves(address(this).balance, _amount);
    ...
    function _logToHub(IMonitoringHub.Severity severity, string memory category, string memory details) internal {
        ...
        if (address(hub) != address(0)) {
            hub.logForensic(IMonitoringHub.ForensicLog({
                ...
                severity: severity, category: category, details: details, riskScore: 10,
                nonce: 0, chainId: block.chainid, blockNumber: block.number, timestamp: block.timestamp,
                gasUsed: gasleft(), value: msg.value, stateRoot: bytes32(0), txHash: bytes32(0),
