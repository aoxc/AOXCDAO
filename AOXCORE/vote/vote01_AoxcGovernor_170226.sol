    ...
    ReentrancyGuard
{
    ...
    function initialize(
        ...
        if (
            address(_token) == address(0) || address(_timelock) == address(0) || _admin == address(0)
                || _repManager == address(0) || _andromeda == address(0)
        ) {
        ...
        returns (uint256)
    {
        ...
        if (address(andromedaCore) != address(0)) {
            if (andromedaCore.getProtocolState() == IAOXCAndromedaCore.ProtocolState.EMERGENCY_PAUSE) {
                return 0;
            }
        }
    ...
    function updateInfrastructure(address _repManager, address _andromeda) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _executeOperations(uint256 pId, address[] memory t, uint256[] memory v, bytes[] memory c, bytes32 d)
        internal
        override(GovernorUpgradeable, GovernorTimelockControlUpgradeable)
        nonReentrant
    {
    ...
    function _queueOperations(uint256 pId, address[] memory t, uint256[] memory v, bytes[] memory c, bytes32 d)
        internal
        override(GovernorUpgradeable, GovernorTimelockControlUpgradeable)
        returns (uint48)
    {
