...
contract AOXCLockManager is ReentrancyGuard, AccessControl, Pausable {
    ...
    // --- Events ---
    event AssetLocked(address indexed user, uint256 indexed batchId, uint256 amount, uint256 duration, uint256 weight);
    ...
    function lock(uint256 _amount, uint256 _duration) external nonReentrant whenNotPaused {
        ...
        // Success
            ...
            _reportToHub(
                IMonitoringHub.Severity.WARNING, "REP_SYNC_FAILURE", "Reputation update failed during lock", 60
            );
        }
    ...
    function unlock(uint256 _batchId) external nonReentrant {
        ...
        // Success
            ...
            _reportToHub(
                IMonitoringHub.Severity.WARNING, "REP_SYNC_FAILURE", "Reputation update failed during unlock", 50
            );
        }
     ...
     */
    function _reportToHub(IMonitoringHub.Severity sev, string memory cat, string memory det, uint8 risk) internal {
