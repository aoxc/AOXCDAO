 ...
 */
contract AOXCTransferAlerts is Initializable, AccessControlUpgradeable, UUPSUpgradeable, AOXCBaseReporter {
    ...
    // --- Events ---
    event TransferAlert(
        address indexed from, address indexed to, uint256 amount, string reason, uint256 timestamp, address reporter
    );
    ...
    function initialize(address admin, address _monitoringHub) external initializer {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "INITIALIZE", "Transfer Alerts Engine online", address(0), 0, ""
        );
    }
        ...
        onlyRole(MONITOR_ROLE)
    {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.CRITICAL, "TRANSFER_ALERT", reason, from, 80, abi.encode(to, amount)
        );
