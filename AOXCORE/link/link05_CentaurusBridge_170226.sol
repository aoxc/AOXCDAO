...
contract CentaurusBridge is AccessControl, ReentrancyGuard {
    ...
    function dispatchData(uint256 _targetChain, bytes32 _dataHash)
        ...
        nonReentrant
    {
        if (!activeRoutes[_targetChain].isVerified) {
            revert RouteNotVerified(_targetChain);
        }
    ...
    function establishRoute(uint256 _chainId, uint256 _strength) external onlyQualified(CENTAURUS_APEX) {
        activeRoutes[_chainId] = LogisticsPath({targetChainId: _chainId, isVerified: true, signalStrength: _strength});

        _log(IMonitoringHub.Severity.WARNING, "DIPLOMACY", "New cross-chain route established.");
        emit RouteEstablished(_chainId, true);
    }

    function heartbeatSignal(uint256 _chainId, uint256 _newStrength) external onlyQualified(CENTAURUS_PULSE) {
