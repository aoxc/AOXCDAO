...
contract AOXCSwap is ReentrancyGuard, Pausable, AccessControl {
    ...
    bytes32 public constant OKB_PRICE_ID = 0x39d15024467d16374971485675e2f782c5f94d9b4b0a48b8b091f86c2d499317;
    ...
    constructor(address _treasury, address _aoxc, address _pyth, address _monitoringHub, address _admin) {
    ...
    function swapOkBtoAoxc(uint256 minAoxcOut) external payable nonReentrant whenNotPaused {
        ...
        if (amountOut < minAoxcOut) {
            _reportToHub(IMonitoringHub.Severity.WARNING, "RISK_ENGINE", "Slippage Violation Blocked", riskScore);
        ...
        if (!success) {
            _reportToHub(IMonitoringHub.Severity.CRITICAL, "FINANCE_CHANNEL", "Treasury Inbound Failed", 99);
    ...
    function _calculateRiskScore(uint256 _in, uint256 _out, uint256 _min) internal view returns (uint8) {
        uint8 score = 10;
