    ...
    AOXCBaseReporter
{
    ...
    function initialize(address admin, address _monitoringHub, address _identity, address _timelock)
        ...
        initializer
    {
        if (admin == address(0) || _monitoringHub == address(0) || _identity == address(0) || _timelock == address(0)) {
        ...
        _performForensicLog(IMonitoringHub.Severity.INFO, "LIFECYCLE", "Bridge Adapter Initialized", address(0), 0, "");
    }
        ...
        returns (bytes32 txHash)
    {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "BRIDGE_LOCK", "Asset Escrowed", recipient, 15, abi.encode(txHash)
        );
    }
     ...
     */
    function finalizeBridge(address, uint256, address, uint256, bytes32 txHash) external onlyTimelock nonReentrant {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "SETTLEMENT", "Bridge Request Finalized", req.owner, 0, abi.encode(txHash)
        );
    }
     ...
     */
    function cancelAndRefund(bytes32 txHash, address token, uint256 amount) external onlyTimelock nonReentrant {
        BridgeRequest storage req = requests[txHash];
        ...
        _performForensicLog(
            IMonitoringHub.Severity.WARNING, "ROLLBACK", "Asset Refund Executed", owner, 5, abi.encode(txHash)
        );
    }
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyRole(UPGRADER_ROLE) {
        _performForensicLog(
            IMonitoringHub.Severity.CRITICAL, "UPGRADE", "Adapter Implementation Migrated", newImplementation, 100, ""
        );
    }
