    ...
    AOXCBaseReporter
{
     ...
     */
    function initialize(address admin, address _monitoringHub, uint256 _initialLimit) external initializer {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "INITIALIZE", "Emergency Transfer Policy deployed", address(0), 0, ""
        );
    }
    ...
    function validateTransfer(address from, address to, uint256 amount) external override {
        ...
        if (frozen) {
            _performForensicLog(IMonitoringHub.Severity.WARNING, "AUTH_REJECT", "Freeze active", from, 40, "");
    ...
    function updatePolicyParameter(string calldata, uint256 newValue) external override onlyRole(ADMIN_ROLE) {
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyRole(UPGRADER_ROLE) {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.CRITICAL, "UPGRADE", "Policy Migration authorized", newImplementation, 100, ""
        );
    }
