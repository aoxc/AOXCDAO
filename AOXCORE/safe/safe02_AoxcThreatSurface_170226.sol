    ...
    AOXCBaseReporter
{
    ...
    // --- Events ---
    event ThreatLogged(uint256 indexed index, IThreatSurface.RiskLevel risk, bytes32 indexed patternId);
    ...
    function initialize(address admin, address _monitoringHub, address _reputationManager) external initializer {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "INITIALIZE", "Threat Consensus Layer Online", address(0), 0, ""
        );
    }
     ...
     */
    function logThreat(string calldata description, IThreatSurface.RiskLevel risk, bytes32 patternId, address suspect)
        external
        ...
        nonReentrant
    {
        ...
        _threats.push(Threat({description: description, risk: risk, timestamp: block.timestamp, reporter: msg.sender}));
        ...
        _performForensicLog(severity, "THREAT_REPORTED", description, suspect, riskScore, abi.encode(patternId, risk));
