...
contract AOXCCircuitBreaker is Initializable, AccessControlUpgradeable, AOXCBaseReporter {
     ...
     */
    function initialize(address admin, address _monitoringHub, address _coordinator, uint256 _initialThreshold)
        external
        initializer
    {
        ...
        _performForensicLog(
            IMonitoringHub.Severity.INFO, "LIFECYCLE", "Circuit Breaker Guard online", address(0), 0, ""
        );
    }
    ...
    function manualReset() external onlyRole(AOXCConstants.ADMIN_ROLE) {
        ...
        _performForensicLog(IMonitoringHub.Severity.WARNING, "MAINTENANCE", "Volume window reset", msg.sender, 10, "");
